...
function tdlp_display_data_shortcode($atts) {
    ...
    // Define filters
    $filters = array('city', 'state', 'country', 'industry', 'job_title');
        ...
        if (!empty($_GET[$filter])) {
            if (is_array($_GET[$filter])) {
                $filter_values = array_map(function($value) use ($wpdb, $filter) {
                    if ($filter === 'job_title') {
                        $simplified_search = strtolower(preg_replace('/[^a-z0-9]+/i', '', $value));
                        return $wpdb->prepare("LOWER(REPLACE(REPLACE(REPLACE($filter, ' ', ''), '-', ''), '_', '')) LIKE %s", '%' . $wpdb->esc_like($simplified_search) . '%');
                    } else {
                ...
                if ($filter === 'job_title') {
                    $simplified_search = strtolower(preg_replace('/[^a-z0-9]+/i', '', $_GET[$filter]));
                    $where_clauses[] = $wpdb->prepare("LOWER(REPLACE(REPLACE(REPLACE($filter, ' ', ''), '-', ''), '_', '')) LIKE %s", '%' . $wpdb->esc_like($simplified_search) . '%');
                } else {
    ...
    // After preparing the query and before executing it, store it in the session
    ...
    <div class="data-filter">
        ...
        <div class="row top align-items-center">
            ...
            <div class="col-lg-3">
           <a id="buy-now-btn"><i class="dashicons dashicons-download"></i>Save This List</a>
            </div>
            ...
            <!-- this button redirect cart page -->
             <button id="buy-now-btn">Buy This List</button>
            </div>
        </div>
        <div class="row mid">
