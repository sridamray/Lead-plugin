...
document.addEventListener('DOMContentLoaded', restoreSelectedValues);
                ...
                <div class="col-xl-9 col-lg-9">
                 ...
                 <div class="pagination">
                        ...
                        $paged = isset($_GET['paged']) ? max(1, intval($_GET['paged'])) : 1;
                        $results_per_page = 18; // Adjust this to your desired number of results per page
                        $offset = ($paged - 1) * $results_per_page;

                     $where_clause = '';

                        // Example of a basic query with pagination (adjust as needed)
                        $query = "SELECT * FROM $table_name $where_clause LIMIT %d OFFSET %d";
                        $prepared_query = $wpdb->prepare($query, $results_per_page, $offset);
                        $results = $wpdb->get_results($prepared_query);
                        

                        // Fetch the total number of filtered results for pagination calculation
                        $total_results_query = "SELECT COUNT(*) FROM $table_name $where_clause";
                        $total_filtered = $wpdb->get_var($total_results_query);
                        $total_pages = ceil($total_filtered / $results_per_page);

                        // Get current query parameters, excluding 'paged'
                        $query_params = $_GET;
                        unset($query_params['paged']);
                        $query_string = http_build_query($query_params);
                        $base_url = get_pagenum_link(1);
                        $base_url = !empty($query_string) ? $base_url . '?' . $query_string . '&paged=%#%' : $base_url . '?paged=%#%';

                        // Display pagination links
                        echo paginate_links([
                            'total' => $total_pages,
                            'current' => $paged,
                            'format' => '?paged=%#%', // Adjust based on your URL structure
                            'base' => $base_url,
